...
/**
  ...
  */
Tree.prototype.getClosestCommonAncestor = function(t1, t2) {
  const p1 = this.getAncestorPath(t1);
  const p2 = this.getAncestorPath(t2);
...
Tree.prototype.getAncestorPath = function(target) {
  let ancestorPath = null, tempPath = [this];
  let recurse = (currNode) => {
    if(currNode === target) {
      ancestorPath = tempPath.slice();
      return
    }
    for (let i = 0; i < currNode.children.length; i++) {
      tempPath.push(currNode.children[i]);
